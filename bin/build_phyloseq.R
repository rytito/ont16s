...
# de las columnas de barcode en la tabla de Emu (barcode01, barcode02, ...).
...
min_reads   <- if (length(args) >= 4) as.numeric(args[4]) else 5000
...
sample_cols <- grep("^barcode", names(raw), value = TRUE)
if (!length(sample_cols)) {
  stop("no se encontraron columnas barcode* en ", counts_file)
}
...
# inestables Y riqueza aparente inflada -- una muestra de 669 lecturas dio el
# mayor número de taxones en una de nuestras corridas. Excluirlas no es
# opcional si planeas ordenar.
...
# GTDB asigna nombres provisionales (p. ej. "JACDBZ01 sp036380955") a
# organismos conocidos por sus genomas pero sin nombre válidamente publicado.
# En un sistema poco estudiado esta fracción es un hallazgo, no un error.
sp <- as.character(tax_table(ps)[, "species"])
placeholder <- grepl("sp[0-9]{6,}|^[A-Z]{2,}[0-9]+", sp) & !is.na(sp)
